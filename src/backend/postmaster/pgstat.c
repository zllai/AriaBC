...
pgstat_get_wait_ipc(WaitEventIPC w)
{
	...
	switch (w)
	{
		...
		case WAIT_EVENT_SYNC_REP:
			event_name = "SyncRep";
			break;
		case WAIT_EVENT_EXEC_WRITE_SET:
			event_name = "ExecWriteSet";
			break;
		case WAIT_EVENT_BLOCK_COMMIT:
			event_name = "BlockCommit";
			break;
		case WAIT_EVENT_TX_FINISH:
			event_name = "TxFinish";
			break;
		case WAIT_EVENT_TX_READY_TO_COMMIT:
			event_name = "TxReadyToCommit";
			break;
		case WAIT_EVENT_OTHER_BLOCK_FINISH:
			event_name = "waitOtherBlockFinish";
			break;
		case WAIT_EVENT_SNAPSHOT_READY:
			event_name = "SnapshotReady";
			break;
			/* no default case, so that compiler will warn */
	}
