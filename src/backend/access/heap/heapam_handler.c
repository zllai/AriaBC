					...
					LockTupleMode *lockmode, bool *update_indexes)
{
	 ...
	 */
	*update_indexes = !HeapTupleIsHeapOnly(tuple);
		...
		for (offnum = FirstOffsetNumber; offnum <= maxoff; offnum = OffsetNumberNext(offnum))
		{
			...
			if (is_bcdb_worker)
			{
				while(true)
				{
					...
					if (!CheckForSerializableConflictOut(valid, scan->rs_rd, &loctup, buffer, snapshot))
						break;
					if (valid)
						ntup--;
				}
				...
				do {
					if (all_visible)
						...
						visible = SampleHeapTupleVisible(scan, hscan->rs_cbuf,
														 tuple, tupoffset);
				} while(CheckForSerializableConflictOut(visible, scan->rs_rd, tuple, hscan->rs_cbuf, scan->rs_snapshot));
			}
			else
			{
				if (all_visible)
