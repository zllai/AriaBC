...
heap_attisnull(HeapTuple tup, int attnum, TupleDesc tupleDesc)
{
	...
	switch (attnum)
	{
		...
		case BCDBBminAttributeNumber:
		case BCDBBmaxAttributeNumber:
	...
	*isnull = false;
	...
	switch (attnum)
	{
		...
		case TableOidAttributeNumber:
			...
			break;
	    case BCDBBminAttributeNumber:
	        result = TransactionIdGetDatum(BCDBHeapTupleHeaderGetBCDBmin(tup->t_data));
	        break;
        case BCDBBmaxAttributeNumber:
	        result = TransactionIdGetDatum(BCDBHeapTupleHeaderGetBCDBmax(tup->t_data));
	        break;
		default:
			elog(ERROR, "invalid attnum: %d", attnum);
