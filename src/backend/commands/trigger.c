					...
					TupleTableSlot *trig_tuple_slot2)
{
	...
	ctid1 = &event->ate_ctid1;
	ctid2 = &event->ate_ctid2;
	...
	switch (event->ate_flags & AFTER_TRIGGER_TUP_BITS)
	{
		...
		default:
			if (ItemPointerIsValid(ctid1))
			{
				...
				if (!table_tuple_fetch_row_version(rel, ctid1,
												   SnapshotAny,
			...
			if ((event->ate_flags & AFTER_TRIGGER_TUP_BITS) ==
				AFTER_TRIGGER_2CTID &&
				ItemPointerIsValid(ctid2))
			{
				LocTriggerData.tg_newslot = ExecGetTriggerNewSlot(estate, relInfo);

				if (!table_tuple_fetch_row_version(rel, ctid2,
												   SnapshotAny,
