...
typedef struct
{
	...
	HASHELEMENT *first_element;  /* to support hash_clear */
	int			cap;
} FreeListData;
...
static bool element_reset(HTAB *hashp, int nelem, int freelist_idx);
...
/* clear shm hash table */
void
shm_hash_clear(HTAB *hashp, long nelem)
{
	...
	/* reset segments */
	for (segp = hashp->dir, i = 0; i < hctl->nsegs; i++, segp++)
		...
		case HASH_ENTER:
			...
			if (currBucket == NULL)
			{
				...
				if (hashp->isshared)
				{
					ereport(ERROR,
							(errcode(ERRCODE_OUT_OF_MEMORY),
							 errmsg("out of shared memory in dynahash %s", hashp->tabname)));
				}
...
get_hash_entry(HTAB *hashp, int freelist_idx)
{
	...
	for (;;)
	{
		...
		if (hashp->hash == dummy_hash)
		{
			/* rs table or ws table */
			if (hctl->freeList[freelist_idx].nentries >= hctl->freeList[freelist_idx].cap)
			{
				SpinLockRelease(&hctl->freeList[freelist_idx].mutex);
				ereport(FATAL, (errmsg("[ZL] out of shared memory in ws or rw table")));
			}
			newElement = (HASHBUCKET)(((char*) hctl->freeList[freelist_idx].first_element) + hctl->freeList[freelist_idx].nentries * elementSize);
...
static bool
element_reset(HTAB *hashp, int nelem, int freelist_idx)
{
	HASHHDR    *hctl = hashp->hctl;
	Size		elementSize;
	...
	firstElement = (HASHELEMENT *) hashp->hctl->freeList[freelist_idx].first_element;
	...
	hctl->freeList[freelist_idx].first_element = firstElement;
	hctl->freeList[freelist_idx].nentries = 0;
...
element_alloc(HTAB *hashp, int nelem, int freelist_idx)
{
	...
	hctl->freeList[freelist_idx].freeList = prevElement;
	hctl->freeList[freelist_idx].first_element = firstElement;
	hctl->freeList[freelist_idx].cap = nelem;
