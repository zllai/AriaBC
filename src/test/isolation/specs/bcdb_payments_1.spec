...
SELECT ctid FROM bank WHERE account = 6;
}
...
step "s1t1" { SELECT bcdb_block_submit('{"bid": 1, "txs": [ 
  {"hash": "1_0", "sql": "SELECT simple_pay(27, 6, 22)"}, 
  {"hash": "1_1", "sql": "SELECT simple_pay(24, 6, 4)"}]}'); }
step "s1t2" { SELECT bcdb_block_submit('{"bid": 2, "txs": [ 
  {"hash": "2_0", "sql": "SELECT simple_pay(21, 2, 22)"},
  {"hash": "2_1", "sql": "SELECT simple_pay(21, 6, 4)"}]}'); }
step "s1t3" { SELECT bcdb_wait_to_finish(); }
step "s1t4" { SELECT bcdb_check_block_status(1); }
step "s1t5" { SELECT * FROM bank; SELECT sum(balance) FROM bank; }

permutation "s1p1" "s1t1" "s1t2" "s1t3" "s1t4" "s1t5"
