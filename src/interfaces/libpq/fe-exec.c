...
/*
 * PQsendBCTx
 *	 Submit a blockchain transaction, but don't wait for it to finish
 *
 * Returns: 1 if successfully submitted
 *			0 if error (conn->errorMessage is set)
 */
int
PQsendBCTx(PGconn *conn, const char *query)
{
	...
	if (pqPutMsgStart('J', false, conn) < 0 ||
		pqPuts(query, conn) < 0 ||
		pqPutMsgEnd(conn) < 0)
	{
	...
	/* remember we are using blockchain transaction protocol */
	conn->queryclass = PGQUERY_BCTX;
...
int
PQBCSendDummy(PGconn *conn, const char *file_path)
{
    if (!PQexecStart(conn))
        return 0;
    if (!PQsendQueryStart(conn))
...
PGresult *
PQexecBCTx(PGconn *conn, const char *query)
{
	if (!PQexecStart(conn))
		return NULL;
	if (!PQsendBCTx(conn, query))
		return NULL;
