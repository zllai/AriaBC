...
typedef struct HeapTupleFields
{
    ...
    BCBlockID bcdb_bmin;  //BCDB inserting block ID
    BCBlockID bcdb_bmax;  //BCDB deleting block ID
...
#define HEAP_BEING_MODIFIED     0x0800  /* bcdb: multiple conflicts on this tuple */
#define HEAP_MULTI_MODIFIED		0x1000  /* bcdb: the tuple is modified by multiple transaction */

#define HeapTupleHeaderIsBeingModified(tup) \
( \
	(tup)->t_infomask2 & HEAP_BEING_MODIFIED \
)

#define HeapTupleHeaderSetBeingModified(tup) \
( \
	AssertMacro(!HeapTupleHeaderXminInvalid(tup)), \
	((tup)->t_infomask2 |= HEAP_BEING_MODIFIED) \
)

#define HeapTupleHeaderIsMultiModified(tup) \
( \
	(tup)->t_infomask2 & HEAP_MULTI_MODIFIED \
)

#define HeapTupleHeaderSetMultiModeifed(tup) \
( \
	AssertMacro(!HeapTupleHeaderXminInvalid(tup)), \
	((tup)->t_infomask2 |= HEAP_MULTI_MODIFIED) \
)
...
#define BCDBHeapTupleHeaderGetBCDBmin(tup) \
( \
	(tup)->t_choice.t_heap.bcdb_bmin \
)

#define BCDHeapTupleHeaderSetBCDBmin(tup, bid) \
( \
	(tup)->t_choice.t_heap.bcdb_bmin = (bid) \
)

#define BCDBHeapTupleHeaderGetBCDBmax(tup) \
( \
	(tup)->t_choice.t_heap.bcdb_bmax \
)

#define BCDHeapTupleHeaderSetBCDBmax(tup, bid) \
( \
	(tup)->t_choice.t_heap.bcdb_bmax = (bid) \
)
